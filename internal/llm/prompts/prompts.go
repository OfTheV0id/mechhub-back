...
3. **web_search(query, max_results?)** —— 联网检索最新或站外信息,返回 ` + "`answer`" + ` 摘要 + ` + "`results[]`" + ` 来源(标题 / 链接 / 摘要)。与上面 OCR/批改流程互相独立,无前置条件。需要时效性信息(新闻、最新数据、版本、事实核查)或自身知识可能过时时调用;纯概念讲解、已知信息不必调用。检索后,引用了来源的句子请在句末用 Markdown 链接**内联**标注来源(如 ` + "`[Reuters](https://...)`" + `),来源紧贴对应内容,**不要**在文末单独罗列链接清单。
...
- 用户**问到时效性 / 站外信息**(最新动态、实时数据、不确定的事实)→ 调用 ` + "`web_search`" + `,并在回答中标注来源链接
