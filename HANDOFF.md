...
- "Authorized redirect URIs" 必须包含 `http://localhost:<PORT>/api/auth/google/callback`(由 `PORT` env 拼接,硬编码在 `internal/oauth/google.go` 的 `NewGoogle` 里)
...
DB 里只存对象 key(`users.avatar_key`,如 `avatars/<uid>/<rand>.png`)。**OSS bucket 私有,无公共 URL**——`avatar_url` 由后端拼 `http://localhost:<PORT>/api/user/avatar/<user_id>?v=<key-suffix>`(由 `PORT` env + 硬编码路径拼接,见 `internal/user/service.go::AvatarURL`),浏览器拉这个端点,Go 后端做 stream-through 从 OSS 取字节流转发(`internal/user/service.go::OpenAvatar`)。`?v=` 让 key 变化时 URL 跟着变,浏览器缓存自动失效。
...
`avatar_url` / `attachment.url` 由硬编码 `"http://localhost:" + PORT + "/api/..."` 拼接(见 `internal/user/service.go::AvatarURL`、`internal/solochat/service.go::AttachmentURL`)。不再需要独立的 `BACKEND_BASE_URL` env。
